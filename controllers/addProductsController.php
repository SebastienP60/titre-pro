...
if(isset($_POST['idFirstSelect'])){
    ...
    if(!empty($_POST['idFirstSelect'])){
        if($_POST['secondSelect'] == 'selectSubcategoriesOfProduct'){
            ...
            $subcategoriesProduct = new subcategoryProduct();
            $subcategoriesProduct->id_ahl115_categories = $_POST['idFirstSelect'];
            //Le echo sert à envoyer la réponse au JS
            echo json_encode($subcategoriesProduct->getListSubcategoriesProduct());
        ...
        $formErrors['selectCategoriesOfProduct'] = 'Veuillez choisir une catégorie pour afficher les champs suivants';
    } 
    ...
    //On instencie dans une variable l'objet de notre model products et de la catégorie
    $product = new product();
        ...
        if(!empty($_POST['selectSubtypeOfProduct'])){
            $product->id_ahl115_subtypes = $_POST['selectSubtypeOfProduct'];
                if(empty($formErrors)){
